...
function lineFromShape({ shape, outlineMat }: { shape: THREE.Shape; outlineMat: THREE.LineBasicMaterial }) {
...
export function buildPaperMesh({ paperMat }: { paperMat: THREE.MeshStandardMaterial }) {
  const paperGeo = new THREE.PlaneGeometry(cfg.geom.paperWidth, cfg.geom.paperHeight, 1, 1);
  const paperMesh = new THREE.Mesh(paperGeo, paperMat);
  paperMesh.position.set(0, 0.3 * cfg.geom.ornamentRadius, 0);
  return paperMesh;
}

export function setInitialPieceTransforms({
  leftOrnamentPiece,
  rightOrnamentPiece,
  capPiece,
}: {
...
export function createOutlineGroup({
  leftOrnamentPiece,
  rightOrnamentPiece,
  capPiece,
  outlineMat,
  ...
  const leftCircleOutline = lineFromShape({ shape: leftOrnamentPiece.circleShape, outlineMat });
  const leftBumpOutline = lineFromShape({ shape: leftOrnamentPiece.bumpShape, outlineMat });
  leftCircleOutline.position.copy(cfg.pts.leftStart);
  leftBumpOutline.position.copy(cfg.pts.leftStart);

  const rightCircleOutline = lineFromShape({ shape: rightOrnamentPiece.circleShape, outlineMat });
  const rightBumpOutline = lineFromShape({ shape: rightOrnamentPiece.bumpShape, outlineMat });
  ...
  group.add(leftCircleOutline, leftBumpOutline, rightCircleOutline, rightBumpOutline, capOutline);
  return group;
}

export function createAssemblyGroups({ scene }: { scene: THREE.Scene }) {
