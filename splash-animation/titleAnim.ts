...
function makeLetterSprite(char: string): THREE.Sprite {
  ...
  sprite.scale.set(cfg.letters.finalScale, cfg.letters.finalScale, 1);
...
//     let cursorX = cfg.ornament.finalPosition.x + cfg.letters.finalScale;

//     for (const ch of cfg.title.letters) {

//       if (ch === " ") {
//         cursorX += cfg.letters.spacing + cfg.letters.wordGapExtra;
//         continue;
//       }
...
createLetters() {
    ...
    for (const ch of cfg.title.letters) {
      const measuredWidthPx = measureGlyphWidth(ch);
      const glyphWidthWorld = measuredWidthPx * cfg.letters.widthScale;
  
      if (ch === " ") {
        cursorX += glyphWidthWorld + cfg.letters.wordGapExtra;
        continue;
      }
